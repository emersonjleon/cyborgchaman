...
from myprompts import openAI_prompt_alargarconpalabras, generar_prompt_alargar_historia, generate_prompt_de_palabras, generar_prompt_crear_historias, historias0
...
*** Colecciones
"""
@app.route("/editarcoleccion/<int:col_idx>", methods=(["GET"]) )
#@app.route("/editarcoleccion/<string:colection_code>", methods=("GET", "POST"))
@login_required    # User must be authenticated
def editarcoleccionpage(col_idx):
    coleccion=Collection.query.filter_by(id=int(col_idx)-42300).first_or_404()
    return render_template("editarcoleccion.html", coleccion=coleccion)


@app.route("/editarcoleccion", methods=( ["POST"]))
#@app.route("/editarcoleccion/<string:colection_code>", methods=("GET", "POST"))
@login_required    # User must be authenticated
def editarcoleccion():
    if request.method == "POST":
        ...
        if myaction == "retirar_historia":  
            ...
            return redirect(url_for("editarcoleccionpage", col_idx=int(col_id)+42300))               
...
def colecciones():
    ...
    if request.method == "POST":
        ...
        if myaction == "nuevacoleccion": 
            ...
            return redirect(url_for("colecciones"))
    colecciones=current_user.is_member_of
...
def leercoleccion(colectionidx):
    ...
    coleccion=Collection.query.filter_by(id=int(col_id)).first_or_404()
    return render_template("leercoleccion.html", coleccion=coleccion)
    ...
    if request.method == "POST":
        ...
        if nuevahistoria=='tokens limit':
            ...
            result = {'prompt':prompt, 'historia':nuevahistoria,
                      'autor':generar_autor(current_user), 'usage':tokens}
...
def crearhistoria():
    if request.method == "POST":
        ...
        if nuevahistoria=='tokens limit':
            ...
            result = {'prompt':prompt, 'historia':nuevahistoria,
                      'autor':generar_autor(current_user), 'usage':tokens_usados}
            result['titulo']=openAI_generar_titulo(result['historia'])
    ...
    if request.method == "POST":
        ...
        if story=='tokens limit':
            ...
            result = {'AIinspiration':palabras, 'prompt':myprompt,
                      'historia': story, 
                      'usage':usage}
            result['autor']= generar_autor(current_user)
            result['titulo']=openAI_generar_titulo(result['historia'])
            h=guardarHistoria(result)
            if current_user.myuseremail=='' and current_user.tokens_usados>TOKENS_EMAIL_REQUEST:
                return render_template("tokensemailrequest.html", tokens_limit=TOKENS_LIMIT, result=h)
...
"""
#########alargarhistoria##############
####alargar con palabras
@app.route("/alargarhistoria/<int:idx>", methods=("GET", "POST"))
@login_required
def requestalargarhistoria(idx):
    if request.method == "POST":
        return redirect(url_for("requestalargarhistoria",idx=idx))
    alargarHistoria=Historia.query.filter_by(id=idx-23500).first_or_404()
    return render_template("alargarhistoria.html", historias=[alargarHistoria])
